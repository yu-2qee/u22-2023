...
public class bank {
	public static void main(String[] args) {
		...
		while(true) {  //ATMの動作開始
			...
			if(menu <= 0 || menu >= 3) {  //範囲外のデータが入力されていないか
		...
		switch(menu) {
		case 1:  //預け入れ
			cardcheck = cardcheck();  //カードの認識
			if(cardcheck == true) {
				passcheck = passcheck(pass);  //パスワードの認証
			}
 
			while(cardcheck == true && passcheck == true) {
	...
	public static boolean cardcheck() {  //カード認識のメソッド
	...
	public static boolean passcheck(String pass) {  //パスワード認証のメソッド
